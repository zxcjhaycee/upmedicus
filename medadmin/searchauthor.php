...
<table id="example" class="table table-striped table-bordered" style="width:100%">
	...
	$query = "SET @num:=0;";
	$query .= "SELECT *, @num:=@num+1 'Row' FROM _tblbib HAVING marc LIKE ('%".$keyword."%') AND marc LIKE '%210%' AND Row = '$fetch'";
...
$MarcFieldsData = substr($ExplodeMarc[0],24,strlen($ExplodeMarc[0]));			
	...
	foreach($values as $key3 => $value3){
		...
		if($tag[$key3] == "100"){
			echo $values[$tag];
			if(strpos($value3, $keyword) !== false){
				// echo $value3;
			}
		}
		...
		//     foreach ($values[$key3] as $key4 => $value4):
		//     	if(strpos($value4, $keyword)):
		//     		if($value3 == '210'):
		//     			echo $values[$key3];
		//     		endif;
		//     	endif;
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$('#prev').on('click', function(){
			...
			pageprev -= 1;
			window.location = "drewdb.php?keyword=<?php echo $keyword ?>&total=<?php echo $total; ?>&fetch_detailed="+pageprev
				});
	}
	if(pagenext == <?php echo $total ?>){
		...
		$('#next').on('click', function(){
			...
			$(this).attr('disabled', false);
			pagenext2++
			window.location = "drewdb.php?keyword=<?php echo $keyword ?>&total=<?php echo $total; ?>&fetch_detailed="+pagenext2
		});
	}
	$('#firstpage').on('click', function(){
		var firstpage = 1;
			window.location = "drewdb.php?keyword=<?php echo $keyword ?>&total=<?php echo $total; ?>&fetch_detailed="+firstpage
	});
		$('#lastpage').on('click', function(){
		var lastpage = <?php echo $total ?>;
			window.location = "drewdb.php?keyword=<?php echo $keyword ?>&total=<?php echo $total; ?>&fetch_detailed="+lastpage
	});
