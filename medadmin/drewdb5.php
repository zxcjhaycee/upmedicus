...
<table id="example" class="table table-striped table-bordered" style="width:100%">
	...
	$primaryidngauthor = "";
	$query = "SET @num:=0;";
	$query .= "SELECT *, @num:=@num+1 'Row' FROM tbl_abstract as abstract LEFT JOIN tbl_abstract_rel as rel ON rel.abstractID = abstract.abstractID  HAVING abstract.abstract LIKE ('%".$keyword."%') AND Row = '$fetch'";
if (mysqli_multi_query($conn, $query)) {
do{
        if ($result = mysqli_store_result($conn)) {
while($santolanfestival = mysqli_fetch_row($result) ){
$primaryidngauthor .= $santolanfestival[3];
$rowngauthor = $santolanfestival[4];
	}
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$('#prev').on('click', function(){
			...
			pageprev -= 1;
			window.location = "drewdb5.php?keyword=<?php echo $keyword ?>&total=<?php echo $total; ?>&fetch_detailed="+pageprev+'&id=<?php echo $primaryidngauthor ?>'
				});
	}
	if(pagenext == <?php echo $total ?>){
		...
		$('#next').on('click', function(){
			...
			$(this).attr('disabled', false);
			pagenext2++
			window.location = "drewdb5.php?keyword=<?php echo $keyword ?>&total=<?php echo $total; ?>&fetch_detailed="+pagenext2+'&id=<?php echo $primaryidngauthor; ?>'
		});
	}
	$('#firstpage').on('click', function(){
		var firstpage = 1;
			window.location = "drewdb5.php?keyword=<?php echo $keyword ?>&total=<?php echo $total; ?>&fetch_detailed="+firstpage+'&id=<?php echo $primaryidngauthor ?>'
	});
		$('#lastpage').on('click', function(){
		var lastpage = <?php echo $total ?>;
			window.location = "drewdb5.php?keyword=<?php echo $keyword ?>&total=<?php echo $total; ?>&fetch_detailed="+lastpage+'&id=<?php echo $primaryidngauthor ?>'
	});
