...
<div class="container">
	...
	<?php
		$inc=4;
		$query=mysqli_query($conn,"select * from tbl_supply where supply_status='0'");
		while($row=mysqli_fetch_array($query)){
			...
			?>
				...
				<div>
					...
					<div style="height: 10px;"></div>
					<div style="display:none; position:absolute; top:210px; left:10px;" class="well" id="cart<?php echo $row['supply_ID']; ?>">Qty: <input type="text" style="width:40px;" id="qty<?php echo $row['supply_ID']; ?>"> <button type="button" class="btn btn-primary btn-sm concart" value="<?php echo $row['supply_ID']; ?>"><i class="fa fa-shopping-cart fa-fw"></i></button></div>
					<div style="margin-left:17px; margin-right:17px;">
						<button type="button" class="btn btn-primary btn-sm addcart" value="<?php echo $row['supply_ID']; ?>"><i class="fa fa-shopping-cart fa-fw"></i> Add to Cart</button> <span class="pull-right"><strong><?php echo ₱number_format($row['Price'],2); ?></strong></span> 
					</div>
				</div>
				</div>
			<?php
